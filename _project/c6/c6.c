...
// 修改者: 陳鍾誠 (模組化並加上中文註解，並將位址區域化 JSUB/JMP/IMM=>Symbol+STR)
...
char *p, *lp, // current position in source code (p: 目前原始碼指標, lp: 上一行原始碼指標)
     *data, *data0;   // data/bss pointer (資料段機器碼指標, data0 為資料段開頭)

int *code0, *e, *le,  // current position in emitted code (code0:機器碼段開頭 e: 目前機器碼指標, le: 上一行機器碼指標)
    ...
    if (tk == '\n') { // 換行
      if (src) {
        ...
        while (le < e) { // 印出上一行的所有目的碼
          ...
          if (*le <= ADJ) {
            if (*le == JSR || *le == JMP) printf(" %04x\n", ((int*)(*++le)-code0)); else printf(" %d\n", *++le); 
          } else 
            printf("\n"); // ADJ 之前的指令都有一個參數，之後都沒有參數。
        }
      }
      ++line;
    }
      ...
      pp = data;
      while (*p != 0 && *p != tk) { // 還沒到字串尾端
        if ((ival = *p++) == '\\') {
          if ((ival = *p++) == 'n') ival = '\n'; // 處理 \n 的特殊情況
        }
        if (tk == '"') *data++ = ival; // 把字元塞到資料段裏
      }
      ++p;
      // c6:if (tk == '"') ival = (int)pp; else tk = Num; // (若是字串) ? (ival = 字串 (在資料段中的) 指標) : (字元值)
      if (tk == '"') ival = (int)(pp-data0); else tk = Num; // (若是字串) ? (ival = 字串 (在資料段中的) 指標) : (字元值)
      return;
    } // 以下為運算元 =+-!<>|&^%*[?~, ++, --, !=, <=, >=, ||, &&, ~  ;{}()],:
...
int run(int *pc, int *bp, int *sp) { // 虛擬機 => pc: 程式計數器, sp: 堆疊暫存器, bp: 框架暫存器
  ...
  while (1) {
    ...
    else if (i == STR) { a = (*pc+data0); pc++; }                         // v6: load string
